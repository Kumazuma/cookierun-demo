...
{
	SHORT debug;
	if (IS_PRESS_KEY_(VK_RIGHT)) {
		DO_IF_IS_VALID_OBJ(m_pGunBarrel) { m_pGunBarrel->RotateCW(); }
		//MoveTo(cfDeltaX[OBJ::DIR_RIGHT] * m_fSpeed, cfDeltaY[OBJ::DIR_RIGHT] * m_fSpeed);
	}
		
	if (IS_PRESS_KEY_(VK_LEFT)) {
		DO_IF_IS_VALID_OBJ(m_pGunBarrel) { m_pGunBarrel->RotateCCW(); }
		//MoveTo(cfDeltaX[OBJ::DIR_LEFT] * m_fSpeed, cfDeltaY[OBJ::DIR_LEFT] * m_fSpeed);
	}

	if (IS_PRESS_KEY_(VK_UP)) {
	...
	for (auto& pShield : m_listShield) { DO_IF_IS_VALID_OBJ(pShield) { pShield->Update(); } }
	...
	DO_IF_IS_VALID_OBJ(m_pGunBarrel) { m_pGunBarrel->Update(); }
